...
1. **Triage rule, PRE-REGISTERED here before any number exists.** See §5.2 — the
   adjudication procedure is fixed now precisely so it cannot be chosen after the
   score is known.
...
## 5.2 Pre-registered triage rule for the discovery-stall stub

Fixed **before** the cold run, so the adjudication cannot be chosen after the
score is visible. This is the whole point of pre-registering it.

**Detection (mechanical, not judgement).** A rep is a *stub rep* iff its generated
script contains **both**
`Generated by supwngo's canonical autopwn pipeline (no technique verified)`
(`supwngo/exploit/pipeline/templates.py:50`) **and** `offset = 0  # TODO`. Both
strings are recorded in `report.json`, so this is decidable from the cold run's own
artifacts with no re-execution. The count of stub reps per target is reported for
every target, pass or fail.

**Classification, by the pattern across that target's 5 reps:**

| pattern across the 5 reps | classification | how it counts in the cold figure |
|---|---|---|
| 0 stub reps, 0 credited | **capability gap** — the pipeline ran a real technique and it did not work | counts as a genuine cold failure |
| 5 stub reps (all) | **discovery failure, cause undetermined** | counts as a cold failure, flagged `discovery-failed`; see adjudication |
| 1–4 stub reps, ≥1 credited | **discovery-stall suspect** — the technique demonstrably exists and works | `solved` = yes; the stub reps are reported in the reliability split *and* flagged, so k/N is not silently read as capability |
| 1–4 stub reps, 0 credited | **discovery-stall suspect, unresolved** | counts as a cold failure, flagged `stall-suspect` |

**Adjudication of a flagged target — decided now, executed after the cold run.**
A flagged target is **re-probed individually**, and that re-probe is
**diagnostic only**:

- It **never** upgrades a cold verdict. The cold figure keeps the target as
  measured. A flagged target that later proves solvable is reported in the
  *diagnosis*, not added to the cold numerator. This is the same firewall the §4
  timeout configuration has, and for the same reason: a re-run under different
  conditions is a different measurement.
- The re-probe is: run that target's `_probe_echo_sizes`-equivalent discovery step
  alone, N=20, on a quiet host, and record the stall rate. A nonzero stall rate on
  a step that succeeds most of the time **identifies the mechanism** as the §5.1
  bug rather than a capability limit.
- The outcome changes **only** (a) the honest narrative of *why* the target failed
  cold, and (b) the priority of the §5.1 fix in step 3.

**Both numbers are published.** The cold figure as measured, and — clearly
separated, never substituted for it — a *"cold figure with discovery-stall
failures excluded from the denominator"* sensitivity figure. The second is
explicitly **not** the headline; it exists so a reader can see how much of the cold
number rests on a known, already-diagnosed infrastructure bug rather than on
capability. Publishing only the first would understate the pipeline; publishing
only the second would launder a real failure. Pre-registering both removes the
choice from me.

**Null-result commitment.** If no target shows the stub, that is stated plainly and
this entire section resolves to "not triggered" — the machinery does not get
repurposed to explain away some other failure mode it was not designed to detect.
...
> **HELD.** The cold run does **not** start until the coordinator sends an explicit
> go signal for host quiescence. This is binding, not advisory. Rationale, which is
> stronger than "be careful": the run uses `--timeout 20`, and §5.1's bug converts
> host load *directly* into apparent capability loss, because `deliver_parts`
> cannot distinguish a stall from a target that printed nothing. Under 4-way agent
> contention the measurement would read load as incapability, and a contaminated
> single-use number cannot be re-taken. The only thing worse than waiting is
> measuring now.
...
## 10.1 The R1↔R2 comparison is NOT like-for-like — state this prominently

The cold figure will inevitably be read against R1's 13/13. The report must
therefore lead with the fact that **the two corpora do not have the same leak
surface**, and that this confound runs in the *opposite* direction to the
overfitting one:

| | R1 | R2 |
|---|---|---|
| flag delivery | compiled in via `-DFLAG` for win()-style targets | **read from `flag.txt` at runtime by every target** |
| `strings <bin> \| grep 'FLAG{'` recovers the flag | **9 of 15 targets** | **0 of 15** (independently verified, §6 step 1.3) |
| targets scoring only behind the bypassable script audit | 9 | 0 |
| `VOID`/excluded | 2 (`11_heap_uaf_leak`, `13_off_by_one`) | TBD by §4.2 |

R2 applied a soundness fix that R1 never received. So R2 is a **strictly
better-built corpus**, and a lower R2 number is partly attributable to R2 being
harder to score *dishonestly*, not only to the pipeline generalizing poorly. Both
effects push the same way and the measurement cannot separate them.

Two consequences, binding on the report:

1. The headline states the comparison is **not** like-for-like, in the headline
   itself rather than in a footnote — a caveat elsewhere in the file does not
   travel with the number that gets quoted.
2. The comparison that *is* closest to like-for-like is **R1-strict vs R2-strict**,
   because `--strict-attribution` is what withholds credit from weak-attribution
   targets. That pairing is reported alongside the default-mode pairing and the
   difference between the two pairings is itself the size of the confound.

This does not rescue the cold number and is not offered as a reason to discount it.
It is a stated limit on what the R1→R2 *delta* can be attributed to.
