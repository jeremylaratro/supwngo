...
- **Normative merge/resolution semantics for the standardized context schema**
  ...
  `docs/reference/context-resolution-tables.md`). This layer had its English
  specification rejected three times for defects prose cannot check, so the
  module — not the prose — is now normative: merge is a validation precondition
  plus `P`/`not P` on one dedup predicate and never picks a winner; provenance
  precedence is a **partial** order generated from four covering edges, which is
  how "an ordinary assertion must not beat a contradictory measurement" becomes
  incomparability demanding an explicit recorded pin rather than a rank that
  always picks someone; scope specificity is a containment DAG (so `libc_file`
  and `host`, being independent axes, are incomparable instead of one
  out-ordering the other for `libc.system_offset`); candidate ids are
  content-derived and every array has a content-derived sort order, which is what
  makes the document digest permutation-invariant; staleness is computed rather
  than stored, so supersession invalidates dependents with no invalidation call
  to forget; and `resolve()` returns one `Selected` or raises one of five named
  refusals, never a default — a fact that was never measured has no candidate,
  because *absent must not be representable as false*. The normative tables are
  generated by calling the runtime comparators (a table backed by parallel
  constants agrees with itself and not with resolution) and byte-gated against
  the committed file. 21 properties assert totality and determinism by exhaustive
  enumeration over a declared, printed bound, and each is paired with one of 29
  named mutants restoring a defect an earlier revision actually shipped, so the
  suite demonstrates the properties can fail rather than asserting it.
  An independent review of the implementation then found eleven mechanical
  defects of exactly the kind prose review cannot reach, all fixed here: a
  candidate id collided with its own retracted sibling after two ordinary
  operations (fixed by a store-assigned `generation` inside the id digest, and
  ids widened from 48 to 128 bits, because one ambiguous id breaks `by_id`,
  pins, dependency lookup, conflict records and array ordering at once);
  `merge()` appended whatever `state` arrived instead of asking the state
  machine, and neither established nor re-checked the store invariants it
  claimed (it is now transactional, validating before and after with rollback);
  validation leaked `TypeError` from inside its own field checks (now funnelled,
  with the one input that only the funnel catches pinned by a test); lifecycle
  writes mutated before validating and permitted self-, cross-key and
  dead-candidate supersession; pins were folded on an operator-supplied
  timestamp, so `"t10"` lost to `"t9"` (now a store-assigned sequence number,
  with validated `pin()`/`unpin()` writers and a mandatory recorded actor,
  since a pin is the one mechanism allowed to override a measurement);
  conflicts were never recorded at write time, so a producer-only run
  accumulated contradictions in silence; and a dependency reference naming the
  right candidate under the wrong key read as fresh. Two of the properties were
  also weaker than claimed — a resolver that refuses everything passed both the
